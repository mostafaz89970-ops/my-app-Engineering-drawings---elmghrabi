...
// حفظ المخطط بشكل دائم ومؤمن 100% يمنع الحذف أو التراجع عند التحديث أو البث (حفظ فوري بدون تأخير)
...
window.getSystemSectorsMap = getSystemSectorsMap;

function getAllSystemAdmins() {
  const map = getSystemSectorsMap();
  const set = new Set();
...
function loadAdminWorkspace(adminName) {
  ...
  // 🛡️ حفظ المخطط الجاري العمل عليه حالياً تلقائياً لحمايته من أي ضياع قبل التبديل
  if (currentProject && Array.isArray(currentProject.nodes) && currentProject.nodes.length > 0) {
    const prevAdmin = currentProject.administration || localStorage.getItem("sld_current_admin") || "بني مزار شرق";
    if (prevAdmin !== "__all__") {
      saveFeederForAdmin(currentProject, prevAdmin);
      try {
        localStorage.setItem("sld_proj_" + currentProject.id, JSON.stringify(currentProject));
        localStorage.setItem("sld_saved_feeder", JSON.stringify(currentProject));
      } catch(_) {}
...
async function saveProjectToStorage(project) {
  ...
  try {
    ...
    const meta = {
      ...
      saved_at: nowTs,
      user_saved_at: nowTs,
      administration: project.administration || curAdmin,
      sector: project.sector || "المنيا شمال",
      is_locked: !!project.is_locked,
      visible_admins: (Array.isArray(project.visible_admins) && project.visible_admins.length > 0)
        ? project.visible_admins
        : [project.administration || curAdmin]
    };
...
async function loadProjectDataById(p_id) {
  ...
  // 5. محاولة جلب المشروع من سحابة Firebase المعتمدة لحفظ ومشاركة المخططات
  try {
    const fbUrl = "https://elmghrabyelectric-default-rtdb.firebaseio.com/sld_studio/projects/" + encodeURIComponent(p_id) + ".json";
    const fbCtrl = new AbortController();
    const fbTimeout = setTimeout(() => fbCtrl.abort(), 2500);
    const fbRes = await fetch(fbUrl, { signal: fbCtrl.signal });
    clearTimeout(fbTimeout);
    if (fbRes.ok) {
      const fbData = await fbRes.json();
      if (fbData && Array.isArray(fbData.nodes)) {
        try {
          localStorage.setItem("sld_proj_" + p_id, JSON.stringify(fbData));
        } catch(_) {}
        return fbData;
      }
    }
...
// تجميع وتأكيد حفظ كافة المشاريع الفعلية في التخزين لضمان ظهور المشروع المحفوظ فوراً
function ensureAllActualProjectsCollected() {
  const curAdmin = getCurrentAdminName();
  const effectiveCurAdmin = (curAdmin && curAdmin !== "__all__") ? curAdmin : "بني مزار شرق";

  // 1. فحص وتثبيت المخطط النشط على الشاشة حالياً (المشروع المحفوظ الآن)
  if (currentProject && Array.isArray(currentProject.nodes) && currentProject.nodes.length > 0) {
    if (!currentProject.id) currentProject.id = "proj_" + (currentProject.saved_at || Date.now());
    if (!currentProject.name || currentProject.name === "مخطط جديد") {
      currentProject.name = currentProject.name || ("مخطط شبكة " + effectiveCurAdmin);
    }
    if (!currentProject.administration || currentProject.administration === "__all__") {
      currentProject.administration = effectiveCurAdmin;
    }
    if (!Array.isArray(currentProject.visible_admins) || currentProject.visible_admins.length === 0) {
  ...
  try {
    const rawSaved = localStorage.getItem("sld_saved_feeder");
    if (rawSaved) {
      const parsedSaved = JSON.parse(rawSaved);
      if (parsedSaved && Array.isArray(parsedSaved.nodes) && parsedSaved.nodes.length > 0) {
        if (!parsedSaved.id) parsedSaved.id = "proj_" + (parsedSaved.saved_at || Date.now());
        if (!parsedSaved.administration || parsedSaved.administration === "__all__") {
          parsedSaved.administration = effectiveCurAdmin;
        }
        if (!Array.isArray(parsedSaved.visible_admins) || parsedSaved.visible_admins.length === 0) {
          parsedSaved.visible_admins = [parsedSaved.administration];
        }
        if (!Array.isArray(parsedSaved.locked_admins)) {
          parsedSaved.locked_admins = parsedSaved.is_locked ? ["__all__"] : [];
        }
        if (!localStorage.getItem("sld_proj_" + parsedSaved.id)) {
          localStorage.setItem("sld_proj_" + parsedSaved.id, JSON.stringify(parsedSaved));
        }
      }
    }
  } catch(_) {}

  // 3. فحص مخططات كافة الفروع sld_feeder_*
    ...
    if (k && k.startsWith("sld_feeder_")) {
      try {
        ...
        if (raw) {
          ...
          if (fObj && Array.isArray(fObj.nodes) && fObj.nodes.length > 0) {
            const admFromKey = k.replace("sld_feeder_", "").replace(/_/g, " ");
            if (!fObj.id) fObj.id = "proj_" + (fObj.saved_at || Date.now());
            if (!fObj.administration || fObj.administration === "__all__") {
              fObj.administration = admFromKey;
            }
            if (!Array.isArray(fObj.visible_admins) || fObj.visible_admins.length === 0) {
              fObj.visible_admins = [fObj.administration];
            }
            if (!Array.isArray(fObj.locked_admins)) {
              fObj.locked_admins = fObj.is_locked ? ["__all__"] : [];
            }
...
window.ensureAllActualProjectsCollected = ensureAllActualProjectsCollected;

// نافذة إدارة المشاريع والمخططات (عرض المشاريع الفعلية فقط مع التصفية السريعة)
async function openProjectsManager(requestedAdmin = null) {
  ...
  modal.classList.remove("hidden");
  container.innerHTML = "<p style='text-align:center; padding:20px; color:#a0aec0;'>جاري فحص وتحديث قائمة المشاريع ومزامنة السحابة...</p>";
  ...
  if (requestedAdmin !== null && requestedAdmin !== undefined) {
    ...
    window.modalViewingAdmin = requestedAdmin;
        ...
        if (raw) {
          ...
          if (p && p.id && !isProjectDeleted(p.id, p.name) && !isDemoOrDummyProject(p)) {
            ...
            masterMap.set(p.id, {
              ...
              nodes_count: (p.nodes || []).length,
              sections_count: (p.sections || []).length,
              updated_at: p.updated_at || new Date().toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' }),
              saved_at: p.saved_at || Date.now(),
              administration: pAdmin,
              sector: p.sector || "المنيا شمال",
              is_locked: (pLocked.length > 0) || !!p.is_locked,
              locked_admins: pLocked,
              visible_admins: pVisible
            });
          }
        }
      } catch(_) {}
  ...
  // ب) جمع المشاريع من فهارس كافة الإدارات sld_catalog_*
      ...
      if (item && item.id && !isProjectDeleted(item.id, item.name) && !isDemoOrDummyProject(item)) {
        if (!masterMap.has(item.id)) {
          ...
          masterMap.set(item.id, {
            ...
            voltage_kv: item.voltage_kv || 11,
            nodes_count: item.nodes_count || 0,
  ...
  if (activeViewingAdmin !== "__all__") {
    try {
      ...
      const catTimeout = setTimeout(() => catController.abort(), 1200);
      ...
      if (catRes.ok) {
        ...
        if (Array.isArray(cloudCat) && cloudCat.length > 0) {
          cloudCat.forEach(cp => {
            if (cp && cp.id && !isProjectDeleted(cp.id, cp.name) && !isDemoOrDummyProject(cp)) {
              if (isProjectVisibleToAdmin(cp, activeViewingAdmin) && !masterMap.has(cp.id)) {
                masterMap.set(cp.id, cp);
              }
            }
          });
        }
...
function renderProjectsTable(projects, isServerOnline, activeViewingAdmin = null) {
  ...
  if (!activeViewingAdmin) {
    activeViewingAdmin = window.modalViewingAdmin || (isAdmin ? curAdmin : userAdmin);
  }
  ...
  const canShareProject = (window.hasPermission && window.hasPermission('share_project')) || isAdmin;

  const allSystemAdmins = (typeof getAllSystemAdmins === "function") ? getAllSystemAdmins() : [
    "بني مزار شرق",
    "بني مزار غرب",
    "مغاغة",
    "العدوة",
    ...
    adminSwitcherHtml = `
      ...
      <div style="display:inline-flex; align-items:center; gap:6px; background:rgba(45, 55, 72, 0.7); padding:4px 10px; border-radius:8px; border:1px solid var(--border-color);">
        ...
        <select id="projects-admin-switcher" style="background:#1a202c; color:#fff; border:1px solid #4a5568; border-radius:6px; padding:3px 8px; font-size:12px; font-weight:600; cursor:pointer;" onchange="changeProjectsModalAdminFilter(this.value)">
          <option value="__all__" ${activeViewingAdmin === "__all__" ? "selected" : ""}>🌍 كل الإدارات (عرض شامل للمدير العام)</option>
          ${adminOptions}
        </select>
      </div>
    `;
      ...
      <table class="projects-table" style="width:100%; border-collapse:collapse;">
        <thead>
          <tr style="background:var(--bg-tertiary); position:sticky; top:0; z-index:2; border-bottom:1px solid var(--border-color);">
            ...
            ${(isAdmin && activeViewingAdmin === "__all__") ? "<th>الإدارة التابع لها</th>" : ""}
            ...
            <th>المقاطع</th>
            <th>تاريخ التعديل</th>
            <th style="min-width:270px; text-align:center;">الإجراءات</th>
          </tr>
        </thead>
        <tbody>
  `;
  ...
  projects.forEach(p => {
    ...
    const sharedSourceBadge = isSharedFromOther
      ? `<span style="margin-right:4px; font-size:9.5px; background:rgba(59,130,246,0.25); color:#93c5fd; border:1px solid #3b82f6; padding:1px 6px; border-radius:8px;" title="المشروع تابع لهندسة ${p.administration} ومصرح بعرضه في هذه الإدارة">🔗 مشاركة من: ${p.administration}</span>`
      : '';

    const multiAdminBadge = (Array.isArray(p.visible_admins) && p.visible_admins.length > 1)
      ? `<span style="margin-right:4px; font-size:9.5px; background:rgba(139,92,246,0.25); color:#c4b5fd; border:1px solid #8b5cf6; padding:1px 5px; border-radius:8px;" title="معروض في إدارات: ${p.visible_admins.join('، ')}">👥 معروض في (${p.visible_admins.length}) إدارات</span>`
      : '';
    ...
    html += `
      <tr style="${rowBg}">
        ...
        ${(isAdmin && activeViewingAdmin === "__all__") ? `<td style="color:#60a5fa; font-weight:600; font-size:11.5px;">${displayAdmin}</td>` : ""}
        <td>${displaySub}</td>
        ...
        <td style="text-align:center;">
          <div style="display:inline-flex; gap:4px; justify-content:center; flex-wrap:wrap; align-items:center;">
            ...
            ${canDeleteProject ? `
              <button class="btn btn-delete btn-sm" style="padding:3px 8px; font-size:11px;" onclick="deleteProjectFromManager(decodeURIComponent('${encId}'), decodeURIComponent('${encName}'))" title="حذف هذا المشروع نهائياً">
                <span>🗑️</span>
              </button>
            ` : ''}
          </div>
...
async function loadProjectFromManager(p_id) {
  ...
  // 🛡️ حفظ المخطط الجاري العمل عليه حالياً على الشاشة أولاً حتى لا يضيع
  if (currentProject && Array.isArray(currentProject.nodes) && currentProject.nodes.length > 0) {
    try {
      localStorage.setItem("sld_proj_" + currentProject.id, JSON.stringify(currentProject));
      localStorage.setItem("sld_saved_feeder", JSON.stringify(currentProject));
  ...
  const targetAdmin = projectData.administration || getCurrentAdminName();
  if (targetAdmin && targetAdmin !== "__all__") {
    localStorage.setItem("sld_current_admin", targetAdmin);
    if (window.currentUser) {
      window.currentUser.administration = targetAdmin;
      try { sessionStorage.setItem("sld_user", JSON.stringify(window.currentUser)); } catch(_) {}
    }
    const formattedTitle = (typeof formatEngineeringTitle === "function") 
      ? formatEngineeringTitle(targetAdmin) 
      : ("هندسة كهرباء " + targetAdmin);
    const sidebarTitle = document.getElementById("sidebar-brand-title");
    if (sidebarTitle) sidebarTitle.textContent = formattedTitle;
    const mainTitle = document.getElementById("main-system-title");
    if (mainTitle) mainTitle.textContent = formattedTitle;
...
async function deleteProjectFromManager(p_id, p_name) {
  if (typeof isCurrentUserAdmin === "function" && !isCurrentUserAdmin() && window.hasPermission && !window.hasPermission('delete_project')) {
    showToast("⛔ ليس لديك صلاحية حذف المشاريع", "error");
    return;
  }
...
function printFullSchematic() {
  ...
  printStyleEl.textContent = `
    @media print {
      ...
      .line-path {
        stroke-width: ${computedLineStroke}px !important;
      ...
      .sld-placard-title,
      .sld-placard-sub,
      .sld-placard-sub tspan,
      .sld-transformer-name,
      .sld-transformer-cap,
      .sld-transformer-id,
      .sld-kiosk-name,
      .sld-kiosk-cap,
      .sld-kiosk-id,
      .sld-node-group text {
        font-size: ${computedFontSize}px !important;
        font-weight: 900 !important;
        fill: #000000 !important;
        stroke: none !important;
      }
      .sld-length-pill {
        stroke-width: ${Math.max(1.5, Math.round(computedLineStroke * 0.25))}px !important;
      }
      .sld-node-group circle,
      .sld-node-group rect {
        stroke-width: ${computedSymbolStroke}px !important;
      }
...
async function submitTransferProject() {
  ...
  updatedProj.visible_admins = keepCopy ? [oldAdmin, newAdmin] : [newAdmin];
  const nowTs = Date.now();
  const nowStr = new Date(nowTs).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' });
  ...
  const meta = {
    ...
    sector: newSector,
    visible_admins: updatedProj.visible_admins
  };
  ...
  // مزامنة فورية مع سحابة Firebase
  try {
    const fbBase = "https://elmghrabyelectric-default-rtdb.firebaseio.com/sld_studio";
    fetch(`${fbBase}/projects/${encodeURIComponent(updatedProj.id)}.json`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(updatedProj)
    }).catch(() => {});
    fetch(`${fbBase}/admins/${encodeURIComponent(newKey)}/catalog.json`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(targetCatalog)
    }).catch(() => {});
...
window.applyHeaderLayout = function(mode) {
  ...
  if (toggleLabel) toggleLabel.textContent = isRibbon ? "النمط الكلاسيكي" : "نمط خاص";
  if (toggleBtn) {
    toggleBtn.title = isRibbon 
      ? "الرجوع فوراً للنمط الكلاسيكي الأصلي ذو الثلاثة صفوف" 
      : "التبديل إلى النمط الخاص المبوب";
  }
...
window.toggleHeaderLayoutMode = function() {
  ...
  if (typeof showToast === "function") {
    if (nextMode === "ribbon") {
      showToast("✨ تم التبديل إلى النمط الخاص المبوب", "info");
    } else {
...
/* ==========================================================================
   Project Lock & Freeze Controller (إيقاف وتجميد المخطط لمنع العبث)
   ========================================================================== */
...
async function saveProjectLockSettings() {
  ...
  if (prModal && !prModal.classList.contains("hidden")) {
    openProjectsManager(window.modalViewingAdmin);
  }
...
/* ==========================================================================
   Multi-Admin Sharing & Visibility Controller (مشاركة وإدارات عرض المشروع)
   ========================================================================== */

async function openProjectSharingModal(pId) {
  if (typeof isCurrentUserAdmin === "function" && !isCurrentUserAdmin() && window.hasPermission && !window.hasPermission('share_project')) {
    showToast("⛔ صلاحية مشاركة وتحديد إدارات عرض المشاريع للمدير العام فقط", "error");
    return;
  }
  ...
  const idInput = document.getElementById("sharing-project-id");
  const nameLabel = document.getElementById("sharing-project-name");
  const container = document.getElementById("sharing-admins-checkboxes-container");
  if (!modal || !idInput || !nameLabel || !container) return;
  ...
  let currentVisible = (Array.isArray(proj.visible_admins) && proj.visible_admins.length > 0)
    ? proj.visible_admins
    : [proj.administration || getCurrentAdminName()];

  if (currentVisible.includes("__all__")) {
    currentVisible = [...allAdmins];
  }

  let htmlSections = "";
  Object.keys(sectorsMap).forEach(sectorName => {
    const branches = sectorsMap[sectorName] || [];
    const branchCheckboxes = branches.map(adm => {
      const isChecked = currentVisible.includes(adm);
      const isPrimary = (adm === proj.administration);
      return `
        <label style="display:flex; align-items:center; gap:8px; padding:6px 10px; background:rgba(30,41,59,0.7); border:1px solid ${isChecked ? '#3b82f6' : '#334155'}; border-radius:6px; cursor:pointer; font-size:12px; color:#f8fafc; transition:all 0.15s ease;">
          <input type="checkbox" name="sharing_admin_cb" value="${adm}" ${isChecked ? "checked" : ""} style="width:16px; height:16px; accent-color:#3b82f6; cursor:pointer;">
          <span>هندسة كهرباء ${adm} ${isPrimary ? '<span style="font-size:10px; color:#ecc94b; font-weight:bold;">(الفرع الأساسي)</span>' : ''}</span>
        </label>
      `;
    }).join('');
    ...
    htmlSections += `
      <div style="margin-bottom:12px;">
        <div style="font-size:12px; font-weight:bold; color:#ecc94b; margin-bottom:6px; border-bottom:1px solid rgba(236,201,75,0.25); padding-bottom:3px; display:flex; align-items:center; gap:6px;">
          <span>📍 قطاع ${sectorName}</span>
        </div>
...
function selectAllSharingAdmins(isSelectAll) {
  const cbs = document.querySelectorAll('input[name="sharing_admin_cb"]');
  cbs.forEach(cb => { cb.checked = isSelectAll; });
}
window.selectAllSharingAdmins = selectAllSharingAdmins;

async function saveProjectSharingSettings() {
  const pId = document.getElementById("sharing-project-id").value;
  const cbs = document.querySelectorAll('input[name="sharing_admin_cb"]:checked');
  const selectedAdmins = Array.from(cbs).map(cb => cb.value);

  if (selectedAdmins.length === 0) {
    showToast("⚠️ يجب اختيار إدارة واحدة على الأقل لعرض المشروع", "warning");
    return;
  }
  ...
  proj.visible_admins = selectedAdmins;
  const nowTs = Date.now();
  const nowStr = new Date(nowTs).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' });
  proj.updated_at = nowStr;

  // حفظ بيانات المخطط في التخزين المحلي
  try {
    localStorage.setItem("sld_proj_" + proj.id, JSON.stringify(proj));
    if (currentProject && (currentProject.id === proj.id || currentProject.name === proj.name)) {
      currentProject.visible_admins = selectedAdmins;
      currentProject.updated_at = nowStr;
      saveFeederForAdmin(currentProject, proj.administration);
      localStorage.setItem("sld_saved_feeder", JSON.stringify(currentProject));
    }
  ...
  const meta = {
    ...
    updated_at: nowStr,
    saved_at: nowTs,
    administration: proj.administration || getCurrentAdminName(),
    sector: proj.sector || "المنيا شمال",
    is_locked: !!proj.is_locked,
    visible_admins: selectedAdmins
  };

  // تحديث فهارس كافة الإدارات: المختارة يُضاف لها المشروع فوراً، وغير المختارة يُزال منها
  allAdmins.forEach(adm => {
    let cat = getCatalogForAdmin(adm) || [];
    const existingIdx = cat.findIndex(c => c.id === proj.id || (proj.name && c.name === proj.name));

    if (selectedAdmins.includes(adm) || adm === proj.administration) {
      if (existingIdx >= 0) {
      ...
      if (existingIdx >= 0) {
        cat.splice(existingIdx, 1);
        saveCatalogForAdmin(cat, adm);
      }
    }
  });

  // مزامنة فورية مع سحابة Firebase المعتمدة
  try {
    const fbBase = "https://elmghrabyelectric-default-rtdb.firebaseio.com/sld_studio";
    // رفع المخطط كاملاً للسحابة لتمكين أي إدارة من فتحه
    fetch(`${fbBase}/projects/${encodeURIComponent(proj.id)}.json`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(proj)
  ...
  // بث سحابي لتحديث الكتالوجات عبر الأجهزة والمتصفحات
  if (typeof window.broadcastProjectSharingUpdated === "function") {
    try { window.broadcastProjectSharingUpdated(proj, selectedAdmins); } catch(_) {}
  } else if (window.broadcastProjectUpdate) {
    try { window.broadcastProjectUpdate("sharing_update", { id: proj.id, visible_admins: selectedAdmins }); } catch(_) {}
  }

  closeProjectSharingModal();
  ...
  showToast(`✅ تم حفظ إدارات العرض بنجاح! المشروع متاح الآن فوراً في ${selectedAdmins.length} إدارات`, "success");
}
window.saveProjectSharingSettings = saveProjectSharingSettings;
